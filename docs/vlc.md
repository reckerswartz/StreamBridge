...
StreamBridge does not control native applications silently. **Open in VLC** is always initiated by a user click: Firefox Android opens the optional bridge handler, while other platforms use their normal share/download behavior.
...
Install the optional `streambridge-*-vlc-bridge.apk`. On Firefox Android, **Open in VLC** opens the bridge's narrow `streambridge-vlc://play` handler directly from that user click; the encoded M3U remains on the device. The bridge:
...
The action is also available for each detected HLS quality. If a master playlist is incompatible with a player, select a specific quality and try its Open in VLC action.
