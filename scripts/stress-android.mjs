...
async function resetTabs(port) {
  ...
  return JSON.parse(await evaluateAddon(port, expression));
}
...
try {
  ...
  for (let index = 1; index <= tabLimit; index += 1) {
    await openUrl(`http://127.0.0.1:8765/stress/control/${index}?capture=1`);
  ...
  await sleep(8000);
  await memorySnapshot("capture-15", port);
  console.log(JSON.stringify({ stage: "capture-15", snapshot: snapshots.at(-1) }));
  ...
  const report = { suite: "android-control", device, avd, startedEmulator, tabLimit, snapshots, failures };
